...
- Package tags: this repository owns the SwiftPM semver tags that clients
  consume. During the current alpha rollout, every published package version is
  emitted as `vX.Y.Z-alpha.N` and can increment on the same upstream stable
  release whenever the packaging pipeline needs another prerelease cut.
...
During the alpha rollout, clients should pin an explicit prerelease version:

```swift
dependencies: [
    .package(url: "https://github.com/<owner>/<repository>.git", exact: "1.6.0-alpha.1")
]
```
...
`Package.swift` is generated by release automation for each published
`vX.Y.Z-alpha.N` tag and points to the matching release assets plus their
checksums.
...
- Published package tags intentionally stay on the alpha prerelease channel
  until the distribution pipeline is considered stable enough for GA tags.
